...
class TimeSeriesDatasetProcessor:
    ...
    Может использовать внешний модуль time_series_utils.py или собственную реализацию.
    """
    ...
    def load_and_preprocess(self):
        ...
        Для задачи классификации также создает метки классов.
        """
    ...
    def value_to_class(self, values):
        ...
        Args:
            values (array-like): Нормализованные или денормализованные значения.

        Returns:
            np.ndarray: Массив целочисленных меток классов.
        """
    ...
    def inverse_transform(self, value):
        ...
        Использует scaler из TSU, если use_tsu=True.

        Args:
            value (array-like): Нормализованные значения.
...
class MLPForecaster:
    ...
    def __init__(self, input_size, hidden_size, output_size, activation_func, learning_rate=0.05, optimizer='SGD', use_dropout=False, dropout_rate=0.0, use_batch_norm=False, task='regression'):
        ...
        Args:
            ...
            output_size (int): Размерность выходного слоя (1 для регрессии, N_classes для классификации).
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs=10000, patience=50, verbose=1, progress_callback=None):
        ...
        Raises:
            ValueError: Если модель не создана (model is None).
        """
    ...
    def predict(self, X):
        ...
        Raises:
            ValueError: Если модель не обучена (model is None).
        """
        if self.model is None:
...
class ProgressCallback(keras.callbacks.Callback):
    """
    Кастомный колбэк для обновления прогресс-бара в tkinter.
    """
    def __init__(self, total_epochs, progress_callback):
        """
        Инициализирует колбэк.

        Args:
            total_epochs (int): Общее количество эпох.
            progress_callback (func): Функция для обновления прогресс-бара (принимает значение 0-100).
        """
        super().__init__()
...
class FuzzyLogicApp:
    ...
    def __init__(self, root):
        ...
        Args:
            root (tk.Tk): Основное окно tkinter.
        """
        self.root = root
    ...
    def load_datasets_params(self):
        ...
        Returns:
            pd.DataFrame or None: DataFrame с параметрами или None, если файл не найден.
        """
        try:
    ...
    def find_dataset_files(self):
        ...
        Returns:
            list: Отсортированный список имён файлов.
        """
        import glob
    ...
    def setup_parameters_frame(self, parent):
        """
        Создает фрейм с параметрами модели на вкладке 'Параметры и Управление'.

        Args:
            parent (tk.Widget): Родительский виджет (вкладка).
        """
    ...
    def setup_control_buttons_frame(self, parent):
        """
        Создает фрейм с кнопками управления на вкладке 'Параметры и Управление'.
    ...
    def setup_log_frame(self, parent):
        """
        Создает фрейм для журнала действий и прогресс-бара.
    ...
    def setup_results_table_frame(self, parent):
        """
        Создает фрейм для вывода результатов и таблицы сравнения.
    ...
    def setup_plots_frame(self, parent):
        """
        Создает фрейм для отображения графиков.
    ...
    def setup_info_frame(self, parent):
        """
        Создает фрейм с описанием датасета 15, кнопками для атрибутов/классов и копирования листинга.
    ...
    def show_attributes(self):
        """
        Показывает окно с информацией о списках атрибутов.
        """
        attr_text = (
            "Список атрибутов:\n"
    ...
    def show_classes(self):
        """
        Показывает окно с информацией о списках названий классов.
        """
        class_text = (
    ...
    def setup_forecast_frame(self, parent):
        """
        Создает фрейм для ввода данных и прогноза на следующий шаг.
    ...
    def log_message(self, message):
        """
        Добавляет сообщение в журнал действий.
    ...
    def browse_dataset(self):
        """
        Открывает диалоговое окно для выбора CSV файла датасета.
        """
    ...
    def on_dataset_combo_change(self, event):
        """
        Обработчик события изменения выбора в Combobox датасета.
        Обновляет dataset_path_var и подставляет рекомендуемые параметры из datasets_parameters.csv.

        Args:
            event: Событие изменения Combobox.
        """
    ...
    def load_and_process_data(self):
        """
        Загружает и обрабатывает данные из выбранного CSV файла.
        Выполняет нормализацию, создание окон, разделение выборки.
        """
        try:
    ...
    def train_model(self):
        """
        Создает и обучает модель на основе текущих параметров и загруженных данных.
        """
    ...
    def evaluate_model(self):
        ...
        Выводит метрики в поле результатов и обновляет current_results.
        """
    ...
    def show_plots_in_new_window(self):
        """
        Открывает новое окно и отображает графики обучения и предсказаний.
        """
    ...
    def save_current_plot(self, fig):
        """
        Сохраняет текущий matplotlib figure как PNG файл.

        Args:
            fig (matplotlib.figure.Figure): Объект фигуры для сохранения.
        """
    ...
    def save_results(self):
        """
        Сохраняет текущие результаты (параметры и метрики) в JSON файл.
        """
    ...
    def add_to_table(self):
        """
        Добавляет текущие параметры и метрики в TreeView таблицы результатов.
        """
        if not self.current_results:
    ...
    def export_table(self):
        """
        Экспортирует содержимое TreeView таблицы в CSV файл.
        """
        filename = filedialog.asksaveasfilename(
    ...
    def show_training_samples(self):
        """
        Показывает первые N примеров обучающей выборки (X и y) в поле результатов.
        N - минимальное из 10 и размера X_train.
        """
        if self.data_processor is None:
            messagebox.showwarning("Предупреждение", "Сначала загрузите и обработайте данные.")
            return
        ...
        try:
            # Берем первые min(10, len(X_train)) строк X_train и y_train
    ...
    def copy_code(self):
        """
        Копирует весь исходный код основного скрипта в буфер обмена.
        """
        try:
    ...
    def save_config(self):
        """
        Сохраняет текущие параметры интерфейса в JSON файл.
        """
        filename = filedialog.asksaveasfilename(
            defaultextension=".json",
    ...
    def load_config(self):
        """
        Загружает параметры интерфейса из JSON файла.
        """
        filename = filedialog.askopenfilename(
    ...
    def run_experiment_series(self):
        """
        Запускает серию экспериментов с различными комбинациями параметров.
        """
    ...
    def _run_experiments_thread(self, hidden_neurons_list, activation_list, depth_list, dataset_list):
        """
        Внутренняя функция для выполнения экспериментов в отдельном потоке.

        Args:
            hidden_neurons_list (list): Список количества нейронов.
            activation_list (list): Список функций активации.
            depth_list (list): Список глубин.
            dataset_list (list): Список файлов датасетов.
        """
    ...
    def save_trained_model(self):
        """
        Сохраняет обученную модель Keras в файл.
        """
    ...
    def save_training_history(self):
        """
        Сохраняет историю обучения (history.history) в JSON файл.
        """
    ...
    def predict_next_step(self):
        """
        Выполняет прогноз следующего значения на основе введенных последних n значений.
        """
        if self.forecaster is None or self.forecaster.model is None:
            messagebox.showwarning("Предупреждение", "Сначала обучите модель.")
            return
